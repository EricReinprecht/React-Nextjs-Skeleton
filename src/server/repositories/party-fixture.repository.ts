...
export async function seedParties(push?: (msg: string) => void) {
    const filePath = path.join(process.cwd(), "src/server/fixtures/partyFixtures.json");
    const imagesDir = path.join(process.cwd(), "public/seed/images");
    const ticketClassFilePath = path.join(process.cwd(), "src/server/fixtures/ticketClassFixtures.json");
    const ticketClassPriceFilePath = path.join(process.cwd(), "src/server/fixtures/ticketClassPriceFixtures.json");
